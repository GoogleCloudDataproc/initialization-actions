...
policies.
Default policy gives admin user full recursive access over all resources in hive
server.
This test creates new database and new empty table that has all permissions
granted to admin user.
Then it creates a policy that will deny using 'SELECT' for 'admin' user on this
empty table.
...
command. This test case
creates yarn service policy which denies submitting applications by default
user.
Tests validates if policy works by submitting example mapreduce job using yarn
jar command.
