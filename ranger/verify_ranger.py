...
This test case creates test directory in /user/username hdfs dir and then creates
policy which denies read access on that test directory for this particular user.
Test validates if policy works by running hdfs dfs -ls command.

3. Testing hive.
By default ranger on dataproc come with the hive service which defines some policies.
Default policy gives admin user full recursive access over all resources in hive server.
This test creates new database and new empty table that has all permissions granted to admin user.
Then it creates a policy that will deny using 'SELECT' for 'admin' user on this empty table.
Test validates if policy works by running hive query using beeline tool.

4. Testing yarn.
By default dataproc user has permission to submit application using yarn command. This test case
creates yarn service policy which denies submitting applications by default user.
Tests validates if policy works by submitting example mapreduce job using yarn jar command.
...
def test_hdfs_plugin():
    ...
    create_policy = "curl --user \"admin:{}\" -H \"Content-Type: application/json\" " \
                    "-X POST -d '{}' http://localhost:{}/service/public/v2/api/policy"\
    ...
    validation_string = 'Permission denied: user={}, access=READ_EXECUTE, inode="/user/{}/test"'\
...
def test_hive_plugin():
    ...
    validate_policy = 'sleep {} && beeline -u jdbc:hive2://localhost:10000 -n admin ' \
                      '-e \'select * from ranger_test_db.ranger_test_table;\''\
        .format(RANGER_POLICY_REFRESH_TIME)

    ret_code, stdout, stderr = run_command(validate_policy)
    validation_string = 'FAILED: HiveAccessControlException ' \
                        'Permission denied: user [admin] does not have [SELECT] privilege'
    if validation_string not in stderr:
