...
public class GoogleCloudStorageReadChannel
 ...
 /**
   ...
   *
   * <p>First, we look at the object's metadata. If no value exists, then we examine the
   * Content-Length header. If neither exists, we then look for and parse the
   * Content-Range header. If there is no way to determine the content length, an
