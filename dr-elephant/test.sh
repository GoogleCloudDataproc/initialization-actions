...
set -x
gsutil cp dr-elephant.sh dr-elephant-env.sh ${bucket_name}/dr-elephant/
cluster_prefix=$1
bucket_name=$2
project_name=$3
# 1.1
...
gcloud dataproc clusters create ${cluster_prefix}-single-${suffix} --subnet default \
  ...
  --image-version ${image} --project ${project_name} \
  --initialization-actions "${bucket_name}/dr-elephant/dr-elephant-env.sh","${bucket_name}/dr-elephant/dr-elephant.sh" &
