...
function test_installation(){
  ...
  do
    sleep 15s 
    curl -i --cacert /usr/lib/knox/data/security/keystores/gateway-identity.pem  -u guest:guest-password -X GET "https://${host_address}:8443/gateway/${topology}/webhdfs/v1/?op=LISTSTATUS" | tee test_output | grep "200 OK"
    [[ $? == 0 ]] && return 0
    cat test_output
  done
...
function test_update_new_topology(){
  gsutil cp /etc/knox/conf/topologies/example-hive-pii.xml gs://${KNOX_GW_CONFIG_GS}/topologies/update_topology.xml
