...
gcloud dataproc clusters create foo --initialization-actions \
  gs://<my-bucket>/conda/bootstrap-conda.sh,gs://<my-bucket>/conda/install-conda-env.sh
```
...
gsutil -m cp -r gs://<my-bucket>/conda/bootstrap-conda.sh .
gsutil -m cp -r gs://<my-bucket>/conda/install-conda-env.sh .
